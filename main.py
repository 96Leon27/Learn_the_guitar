...
def read_text(relative_path, default=""):
...
def add_shadow(widget, blur=35, alpha=130, y=8):
...
class AnimatedBackground(QWidget):
    ...
    def paintEvent(self, event):
        ...
        self.draw_guitar(painter, w, h)
        if self.show_chords:
            self.draw_chords(painter, w, h)
        if self.show_wave:
            self.draw_wave(painter, w, h)
        if self.show_equalizer:
            self.draw_equalizer(painter, w, h)

    def draw_particles(self, p, w, h):
...
class MenuBackgroundMusic(QtCore.QObject):
    ...
    def scantracks(self):
        ...
        for audio_path in files:
            cover_path = self.cover_for(audio_path.stem)
            self.tracks.append(
                MenuBackgroundTrack(
    ...
    def select_track(self, index, restart=True):
    ...
    def set_user_volume_percent(self, value):
...
_MENU_BACKGROUND_MUSIC = None


def get_menu_background_music():
            ...
            for index, track in enumerate(self.manager.tracks):
                ...
                if index == self.manager.current_index:
                    button.setStyleSheet(
                        button.styleSheet() + f"background: rgba(255,179,92,0.16); border-color: rgba(255,179,92,0.38);")
                button.clicked.connect(lambda checked=False, i=index: self.select_track(i))
...
class TrackTable(QTableWidget):
    ...
    def set_item(self, row, column, text, track_id, center=False, stars=False):
        ...
        item.setTextAlignment(
            Qt.AlignmentFlag.AlignVCenter | (Qt.AlignmentFlag.AlignCenter if center else Qt.AlignmentFlag.AlignLeft))
